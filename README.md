...
| [Ollama](https://ollama.com/) | latest | 임베딩 모델 (bge-m3) |
...
`install.sh`가 8단계를 자동 실행합니다:
1. 필수 도구 확인
2. 소스 배치 + `.env` 생성
3. Docker 컨테이너 시작 (Qdrant + MinIO)
4. `npm install` + TypeScript 빌드
5. Ollama 모델 풀 (bge-m3)
6. Claude Code `settings.json` MCP 등록
7. Skill 파일 설치
8. 헬스체크 검증
...
| `OLLAMA_URL` | | `http://localhost:11434` | Ollama 서비스 URL |
| `EMBEDDING_MODEL` | | `bge-m3` | 임베딩 모델 (1024차원) |
...
│   │   ├── embeddingService.ts  # Ollama 임베딩
...
| [Ollama](https://ollama.com/) | 로컬 임베딩 모델 (bge-m3) |
