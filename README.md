...
> 서버/클라이언트는 동일 코드. `.env`의 `CACHE_ENABLED`로 구분한다.
...
| **설정** | `./install.sh` 만으로 완료 | `.env`에 `CACHE_ENABLED=true` + `ZIME_SSH_HOST` |
...
> 수동 설치는 아래 [Installation](#installation) 참고. 원격 접속 모드(SSH 터널)는 [원격 접속 설정](#원격-접속-설정-클라이언트-모드) 참고.
...
> openssl rand -hex 32
> ```
> 생성된 값을 `.env`의 `MINIO_SECRET_KEY`, `MINIO_ROOT_PASSWORD`, `ZIME_ENCRYPTION_KEY`에 입력하세요.
